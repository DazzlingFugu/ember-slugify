...
module('Unit | Utility | slug', function () {
  ...
  test('tests on slugify with options', function (assert) {
    assert.strictEqual(slugify('Bon jOuR Töi 1337', { replacement: '#' }), 'bon#jour#toi#1337')
    assert.strictEqual(slugify('Bon jOuR Töi 1337', { lower: false }), 'Bon-jOuR-Toi-1337')
    ...
    assert.strictEqual(slugify(' keep the spaces ', { trim: false }), '-keep-the-spaces-')
    assert.strictEqual(
